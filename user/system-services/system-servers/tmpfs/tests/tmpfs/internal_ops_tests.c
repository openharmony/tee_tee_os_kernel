...
void init_test_root()
{
    ...
    tmpfs_test_root->d_ops->add_dentry(tmpfs_test_root, d_root_dotdot, "..", 2);

    tmpfs_test_root->d_ops->link(tmpfs_test_root, d_root_dot, tmpfs_test_root);
                                        ...
                                        struct dentry *dentry, mode_t mode)
{
    ...
    if (err) {
        should_free[2] = dotdot, should_free[1] = dot, should_free[0] = new_dir,
        free_idx = 2;
        goto free_dotdot;
    }
...
MU_TEST(base_ops_tests)
{
    ...
    for (int i = 0; i < TEST_N; i++) {
        ...
        inode = tmpfs_inode_init(rand() % 3 + 1, 0);
...
MU_TEST(reg_ops_tests)
{
    ...
    for (int i = 0; i < TEST_N; i++) {
        ...
        size = reg->size;
        expected_size = size > write_len + write_off ? size :
                                                       write_len + write_off;
        ...
        for (int j = 0; j < TEST_N; j++) {
            ...
            if (read_off >= reg->size) {
                ...
                expected_ret = read_len + read_off <= reg->size ?
                                   read_len :
                                   reg->size - read_off;
            }
    ...
    for (int i = 0; i < TEST_N; i++) {
        ...
        case 1:
            ...
            memset(file_content + reg->size, 0, MAX_TEST_FILE_SIZE - reg->size);
            break;
        case 2:
...
MU_TEST(dir_ops_tests)
{
    ...
    for (int i = 0; i < TEST_N * TEST_N * TEST_N; i++) {
        ...
        mu_check(
            tmpfs_test_root->d_ops->is_empty(tmpfs_test_root)); /* is_empty:
                                                                   true */
        ...
        d_found =
            tmpfs_test_root->d_ops->dirlookup(tmpfs_test_root, test_name, len);
         ...
         */
        tmpfs_test_root->d_ops->mknod(tmpfs_test_root, d_test, 0, FS_REG);
        i_test = d_test->inode;
        ...
        while ((err = tmpfs_test_root->d_ops->mkdir(tmpfs_test_root, d_test, 0))
               != 0) {
        ...
        struct dentry *dotdot = i_test->d_ops->dirlookup(i_test, "..", 2);
        ...
        i_test->d_ops->add_dentry(
            i_test, d_tmpfile_renamed, tmpfile_renamed_name, renamed_len);
        ...
        struct dentry *d_tmpfile_renamed_found =
            i_test->d_ops->dirlookup(i_test, tmpfile_renamed_name, renamed_len);
        ...
        should_free[0] = d_tmpfile, free_idx = 0; /* d_tmpfile will be
                                                     freed */
        i_test->d_ops->rename(i_test, d_tmpfile, i_test, d_tmpfile_renamed);
        mu_check(free_idx == -1);

        d_tmpfile_found = i_test->d_ops->dirlookup(i_test, tmpfile_name, len);
        d_tmpfile_renamed_found =
            i_test->d_ops->dirlookup(i_test, tmpfile_renamed_name, renamed_len);
        ...
        mu_check(
            i_test->d_ops->dirlookup(i_test, tmpfile_renamed_name, renamed_len)
            == d_tmpfile_renamed);
        mu_check(i_test->d_ops->rmdir(tmpfs_test_root, d_test) == -ENOTEMPTY);
        ...
        should_free[3] = dot, should_free[2] = dotdot, should_free[1] = d_test,
        should_free[0] = i_test, free_idx = 3;
        mu_check(tmpfs_test_root->d_ops->rmdir(tmpfs_test_root, d_test) == 0);
        mu_check(free_idx == -1);

        mu_check(
            tmpfs_test_root->d_ops->is_empty(tmpfs_test_root)); /* / is empty */
        ...
        mu_check(err == 0);
        err = tmpfs_test_root->d_ops->mknod(tmpfs_test_root, d_tmp, 0, FS_REG);
        mu_check(err == 0);
        ...
        /* creating /test */
        d_test = tmpfs_test_root->d_ops->alloc_dentry();
        tmpfs_test_root->d_ops->add_dentry(tmpfs_test_root, d_test, "test", 4);

        /* since mkdir can fail in the test.. */
        while (tmpfs_test_root->d_ops->mkdir(tmpfs_test_root, d_test, 123)) {
            ;
        }
        ...
        should_free[1] = d_renamed, should_free[0] = i_renamed, free_idx = 3;
        err = i_dir2->d_ops->rmdir(i_dir2, d_renamed);
        mu_check(!err);
        mu_check(free_idx == -1);

        struct dentry *dir2_dot = i_dir2->d_ops->dirlookup(i_dir2, ".", 1);
        struct dentry *dir2_dotdot = i_dir2->d_ops->dirlookup(i_dir2, "..", 2);
        ...
        struct dentry *test_dot = i_test->d_ops->dirlookup(i_test, ".", 1);
        struct dentry *test_dotdot = i_test->d_ops->dirlookup(i_test, "..", 2);
