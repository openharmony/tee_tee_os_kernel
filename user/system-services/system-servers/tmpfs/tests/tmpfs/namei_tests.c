...
static void this_is_valid_reg()
{
    ...
    test_mknod(
        i_thisisvalid, d_thisisvalid, &d_thisisvalidreg, FS_REG, "reg", 3);
...
static void this_sym0tovalid_absolute()
{
    ...
    test_mknod(i_this, d_this, &d_this_sym0tovalid, FS_SYM, "sym0tovalid", 11);
    i_this_sym0tovalid = d_this_sym0tovalid->inode;
    i_this_sym0tovalid->sym_ops->write_link(i_this_sym0tovalid, symlink, len);
}
...
static void this_sym1tovalid_relative()
{
    ...
    size_t len = strlen(symlink);
    test_mknod(i_this, d_this, &d_this_sym1tovalid, FS_SYM, "sym1tovalid", 11);
    i_this_sym1tovalid = d_this_sym1tovalid->inode;
    i_this_sym1tovalid->sym_ops->write_link(i_this_sym1tovalid, symlink, len);
}
...
MU_TEST(lookupat_tests)
{
     ...
     */
    err = path_lookupat(&nd, "/this/sym0tovalid/reg", ND_NO_SYMLINKS, &d_found);
    mu_check(err == -ELOOP);
...
MU_TEST(openat_tests)
{
     ...
     */
    err = path_openat(&nd, "/this/is/valid//new_dir///", O_CREAT, 0, &d_found);
    mu_check(err == -EISDIR);
    ...
    err =
        path_openat(&nd, "///this////is///valid////", O_DIRECTORY, 0, &d_found);
    mu_check(err == 0);
    mu_check(d_found->inode == i_thisisvalid);
    ...
    err = path_openat(&nd, "/this///is//valid//.", O_DIRECTORY, 0, &d_found);
    mu_check(err == 0);
    mu_check(d_found->inode == i_thisisvalid);
    ...
    for (int i = 0; i < 200; i++) {
        ...
        can_fail = true;
        /* loop until succeed */
        while ((err = path_openat(&nd, name, O_CREAT, 0, &d_found)) != 0) {
            mu_check(err == -ENOMEM);
        }
        can_fail = false;
