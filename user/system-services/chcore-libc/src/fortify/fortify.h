...
#define __DIAGNOSE__SIZE_MUL_OVERFLOW(a, b, result) \
    __builtin_umull_overflow(a, b, result)
#define __DIAGNOSE_PRINTFLIKE(x, y)           __attribute__((__format__(printf, x, y)))
#define __DIAGNOSE_CALL_BYPASSING_FORTIFY(fn) (&(fn))
#define __DIAGNOSE_FORTIFY_INLINE                         \
...
#define __DIAGNOSE_PASS_OBJECT_SIZE \
    __DIAGNOSE_PASS_OBJECT_SIZE_N(__DIAGNOSE_BOS_LEVEL)
#define __DIAGNOSE_PASS_OBJECT_SIZE0 __DIAGNOSE_PASS_OBJECT_SIZE_N(0)

#define __DIAGNOSE_FORTIFY_UNKNOWN_SIZE ((unsigned int)-1)
/* The following are intended for use in unevaluated environments, e.g.
 * diagnose_if conditions. */
...
#define __DIAGNOSE_BOS_TRIVIALLY_GE(bos_val, index) \
    __DIAGNOSE_BOS_DYNAMIC_CHECK_IMPL((bos_val), >=, (index))
#define __DIAGNOSE_BOS_TRIVIALLY_GT(bos_val, index) \
    __DIAGNOSE_BOS_DYNAMIC_CHECK_IMPL((bos_val), >, (index))
...
#define __DIAGNOSE_OPEN_MODES_USEFUL(flags) \
    (((flags)&O_CREAT) || ((flags)&O_TMPFILE) == O_TMPFILE)
#define __DIAGNOSE_BOS_FD_COUNT_TRIVIALLY_SAFE(bos_val, fds, fd_count) \
    __DIAGNOSE_BOS_DYNAMIC_CHECK_IMPL_AND(                             \
        (bos_val),                                                     \
        >=,                                                            \
        (sizeof(*(fds)) * (fd_count)),                                 \
        (fd_count) <= __DIAGNOSE_CAST(static_cast, unsigned int, -1)   \
                          / sizeof(*(fds)))
...
#define OPEN_USELESS_MODES_WARNING \
    "having redundant mode bits; but missing O_CREAT."
#define CALLED_WITH_STRING_BIGGER_BUFFER \
    "called with a string larger than the buffer"
...
#define OUTPUT_PARAMETER_BYTES \
    "the output parameter must be nullptr or a pointer to the buffer with >= FORTIFY_PATH_MAX bytes"
#define SIZE_LARGER_THEN_DESTINATION_BUFFER \
    "the size is greater than the target buffer"
