...
int chcore_shmget(key_t key, size_t size, int flag)
{
    ...
    shm_msg = ipc_create_msg(shmmgr_ipc_struct, sizeof(struct shm_request));
...
void *chcore_shmat(int id, const void *addr, int flag)
{
    ...
    if (!addr) {
        shmaddr = chcore_auto_map_pmo(shm_cap, shm_req->result.size, perm);
        if (shmaddr == NULL) {
            /* errno is set in chcore_auto_map_pmo */
            ret = -1;
        }
    } else {
        ret = usys_map_pmo(SELF_CAP, shm_cap, (unsigned long)addr, perm);
        shmaddr = (void *)addr;
    }
    if (ret < 0) {
        errno = ret;
