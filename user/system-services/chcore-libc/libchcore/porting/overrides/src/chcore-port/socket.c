...
static void translate_sockaddr_in_to_lwip(struct lwip_sockaddr_in *output,
                                          const struct sockaddr_in *input)
{
     ...
     * of these parts.
     */
    memcpy(&output->sin_port,
           &input->sin_port,
           field_to_end_size(struct sockaddr_in, sin_port));
}

static void translate_sockaddr_in6_to_lwip(struct lwip_sockaddr_in6 *output,
                                           const struct sockaddr_in6 *input)
{
     ...
     * of these parts.
     */
    memcpy(&output->sin6_family,
           &input->sin6_family,
           field_to_end_size(struct sockaddr_in6, sin6_port));
}

static size_t translate_copy_to_lwip(const struct sockaddr *input,
                                     socklen_t input_len, void *buf)
{
...
static int __lwip_ipc_sockaddr(ipc_msg_t **ipc_msg_p, enum LWIP_REQ req,
                       ...
                       socklen_t *restrict len)
{
    ...
    if (fd_dic[fd] == 0)
        return -EBADF;
    ret = __lwip_ipc(
        &ipc_msg, LWIP_SOCKET_NAME, NULL, 0, 2, fd_dic[fd]->conn_id, *len);
    lr_ptr = (struct lwip_request *)ipc_get_msg_data(ipc_msg);
    *len = lr_ptr->args[1];
                       ...
                       socklen_t *restrict len)
{
    ...
    if (fd_dic[fd] == 0)
        return -EBADF;
    ret = __lwip_ipc(
        &ipc_msg, LWIP_SOCKET_PEER, NULL, 0, 2, fd_dic[fd]->conn_id, *len);
    lr_ptr = (struct lwip_request *)ipc_get_msg_data(ipc_msg);
    *len = lr_ptr->args[1];
    BUG_ON(*len > LWIP_DATA_LEN);
    if (addr != 0 && *len != 0)
...
int chcore_bind(int fd, const struct sockaddr *addr, socklen_t len)
{
    if (fd_dic[fd] == 0)
        return -EBADF;
    BUG_ON(len > LWIP_DATA_LEN);
    return lwip_ipc_sockaddr(
        LWIP_SOCKET_BIND, addr, len, 2, fd_dic[fd]->conn_id, len);
}
...
int chcore_listen(int fd, int backlog)
{
    if (fd_dic[fd] == 0)
        return -EBADF;
    return lwip_ipc(LWIP_SOCKET_LIST, NULL, 0, 2, fd_dic[fd]->conn_id, backlog);
}
                  ...
                  socklen_t *restrict len)
{
    ...
    ret = __lwip_ipc(
        &ipc_msg, LWIP_SOCKET_ACPT, NULL, 0, 2, fd_dic[fd]->conn_id, *len);
    lr_ptr = (struct lwip_request *)ipc_get_msg_data(ipc_msg);
...
int chcore_connect(int fd, const struct sockaddr *addr, socklen_t len)
{
    if (fd_dic[fd] == 0)
        return -EBADF;
    return lwip_ipc_sockaddr(
        LWIP_SOCKET_CONN, addr, len, 2, fd_dic[fd]->conn_id, len);
}
                  ...
                  const struct sockaddr *addr, socklen_t alen)
{
    ...
    if ((fd_dic[fd]->private_data != NULL)
        ...
        if (alen != 0) {
            if (addr->sa_family != AF_INET6 && addr->sa_family != AF_INET) {
                ...
                size_t buflen =
                    translate_copy_to_lwip(addr, alen, &sockaddr_buf);
    ...
    /* Else, separate to multiple IPC*/
        ...
        while (remain > 0 && ret == copylen) {
            /* If remain cannot send in one ipc, leave it to the
             * next ipc */
            copylen = remain + alen > LWIP_DATA_LEN ? LWIP_DATA_LEN - alen :
                                                      remain;
                    ...
                    struct sockaddr *restrict addr, socklen_t *restrict alen)
{
    ...
    if ((len + alen_val > LWIP_DATA_LEN) && (fd_dic[fd]->private_data != NULL)
        && (*((int *)fd_dic[fd]->private_data) == SOCK_DGRAM))
        ipc_msg = ipc_create_msg_with_cap(
            lwip_ipc_struct, sizeof(struct lwip_request), 1);
    else
