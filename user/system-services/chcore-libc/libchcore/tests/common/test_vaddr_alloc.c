...
struct range ranges[] = {
    {MEM_AUTO_ALLOC_REGION + 0x1000, 0x1000},
    {MEM_AUTO_ALLOC_REGION + 0x3000, MEM_AUTO_ALLOC_REGION_SIZE - 0x6000},
    {MEM_AUTO_ALLOC_REGION + MEM_AUTO_ALLOC_REGION_SIZE - 0x2000, 0x1000}};
...
MU_TEST(vaddr_alloc_basic)
{
    ...
    mu_check(ret == MEM_AUTO_ALLOC_REGION);
    ...
    ret = chcore_alloc_vaddr(0x1000);
    mu_check(ret == MEM_AUTO_ALLOC_REGION + 0x2000);

    ret = chcore_alloc_vaddr(0x1000);
    mu_check(ret
             == MEM_AUTO_ALLOC_REGION + MEM_AUTO_ALLOC_REGION_SIZE - 0x3000);

    ret = chcore_alloc_vaddr(0x1000);
    mu_check(ret
             == MEM_AUTO_ALLOC_REGION + MEM_AUTO_ALLOC_REGION_SIZE - 0x1000);

    chcore_free_vaddr(MEM_AUTO_ALLOC_REGION, 0x1000);

    chcore_free_vaddr(
        MEM_AUTO_ALLOC_REGION + MEM_AUTO_ALLOC_REGION_SIZE - 0x1000, 0x1000);
...
{
    vaddr_t ret;
    vaddr_t start = MEM_AUTO_ALLOC_REGION;
    vaddr_t end = MEM_AUTO_ALLOC_REGION + MEM_AUTO_ALLOC_REGION_SIZE;
    int allocated_cnt = 0;
    int remains;
