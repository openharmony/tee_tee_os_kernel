...
struct answer_node* answer_struct_get(struct answer_struct* this, int key)
{
    if (!this->answer_slots[key]) {
        ...
        return container_of(this->answer_slots[key], struct answer_node, node);
    }
}
...
MU_TEST(test_fuzzing_rbtree)
{
    ...
    for (int i = 0; i < TEST_ROUNDS; i++) {
        ...
        case 1: {
            ...
            struct answer_node* answer = answer_struct_get(answers, key);
            struct rb_node* rnode = rb_search(tree, &key, tree_cmp_key);
            if (answer) {
        ...
        case 2: {
            ...
            mu_check(r);
            struct tree_node* tree_node = rb_entry(r, struct tree_node, node);
